...
One can refer to a column in the format `tableName.columnName`.
...
`ORDER` is not used by itself but is often followed by `BY` to sort results.
...
`ON` specifies the join condition when using JOIN operations.
...
`OR` allows you to retrieve rows that meet either of two conditions in a WHERE clause.
...
`NOT IN` retrieves rows that do not match any value in a list.
...
`BETWEEN` is used to filter rows within a specified range.
...
`NOT BETWEEN` filters rows outside a specified range.
...
`ASC` and `DESC` are used with `ORDER BY` to specify ascending or descending sorting.
...
`OFFSET` is used to skip a specified number of rows before retrieving results.
...
`NULL` is used to represent missing or unknown data in SQL queries.
...
`AS` is used to alias columns or tables, providing a temporary name for them in the result set.
...
COUNT(\*) is an SQL function used to count the total number of rows in a table, including rows with NULL values.

For example, to count the number of rows in a "students" table:
...
INSERT INTO is an SQL statement used to insert new records (rows) into a table.

For example, to insert a new student into a "students" table:
...
In SQL, VALUES is used to specify the data that you want to insert into a table when using the INSERT INTO statement.

For example, in the following SQL statement, "John" and "Doe" are values for the "first_name" and "last_name" columns:
...
UPDATE is an SQL statement used to modify existing records in a table.

For example, to update the age of a student with the ID of 1:
...
SET is used with the UPDATE statement to specify the columns you want to modify and the new values you want to assign to those columns.

For example, in the following SQL statement, "age = 26" sets the "age" column to 26:
...
DELETE FROM is an SQL statement used to delete records from a table based on specified conditions.
...
CREATE TABLE IF NOT EXISTS is an SQL statement used to create a new table if it doesn't already exist.

For example, to create a "students" table if it doesn't exist:
...
ALTER TABLE is an SQL statement used to modify an existing table structure, such as adding, modifying, or deleting columns.

For example, to add a new "email" column to a "students" table:
...
DROP is an SQL statement used to delete database objects like tables, indexes, or views.

For example, to delete a "students" table:
...
RENAME TO is used to rename an existing table in SQL.

For example, to rename a "old_table" to "new_table":
...
AND is a logical operator in SQL used to combine multiple conditions in a WHERE clause. All conditions must be true for a row to be included in the result set.

For example, to retrieve students who are both male and have an age greater than 18:
...
DROP TABLE IF EXISTS is an SQL statement used to delete a table if it exists, helping to avoid errors when trying to delete a non-existent table.

For example, to delete a "students" table if it exists:
...
DDL (Data Definition Language) commands are used to define the database schema. It simply deals with descriptions of the database schema and is used to create or modify the structure of database objects in the database.
...
DML (Data Manipulation Language) commands deal with the manipulation of data present in the database. It is the component of the SQL statement that controls the access to data and to the database.
...
TCL (Transaction Control Language) commands are used to control the start and end of transactions and to ensure data consistency and integrity in a database.
